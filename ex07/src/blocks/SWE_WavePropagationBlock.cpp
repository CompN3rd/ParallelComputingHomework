	...
	hvNetUpdatesAbove (nx, ny + 1)
{
	...
	dims = new int[2];
	dims[0] = (int)sqrtf(numProcessors);
	dims[1] = (int)sqrtf(numProcessors);

	int periods[2] = {false, false};

	//check correct divisibility
	if( dims[0] * dims[1] != numProcessors ||
		nx % dims[0] != 0 ||
		ny % dims[1] != 0)
	{
		std::cerr << "Can't subdivide this simulation domain " << nx << " x " << ny << " with this number of processors: " << dims[0] << " x " << dims[1] << std::endl;
		exit(1);
	}
